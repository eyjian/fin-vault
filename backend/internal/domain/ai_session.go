...
//
// 索引设计：
//   - idx_session_created (f_session_id, f_created_at)：按会话拉取步骤时间线
...
type AgentStep struct {
	ID        string          `gorm:"column:f_id;type:varchar(36);primaryKey" json:"id"`
	SessionID string          `gorm:"column:f_session_id;type:varchar(36);not null;index:idx_session_created,priority:1;index:idx_step_session" json:"session_id"`
	...
	CreatedAt time.Time       `gorm:"column:f_created_at;not null;index:idx_session_created,priority:2;index:idx_created" json:"created_at"`
}
