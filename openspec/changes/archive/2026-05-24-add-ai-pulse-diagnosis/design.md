...
**决策**：采用**紧凑方案**——在资产管理列表中：
- 操作列前新增 1 列"AI 把脉"，用 `el-tag` 显示结论（彩色映射：sell→danger / reduce→warning / hold→success / add→primary）
- tag 旁带置信度小标记：low 时显示灰色感叹号图标 + tooltip "请谨慎参考"
- 点击 tag 弹出 popover，分层展示：先 summary（≤80 字简要），点"查看详细分析"再展开 detail（≥150 字含术语解释）
- 操作栏新增"AI 把脉"图标按钮（魔法棒），位于"录流水"前

**理由**：当前列表已有 17 列，再加多列会破坏可读性；popover + 分层展开既保持紧凑，又契合"分层原因"的产品设计；与现有 el-tag 交互风格一致。

**备选方案（已弃用）**：
- B) 新增 3 列（结论 / 置信度 / 简要原因）→ 列数 +3，UI 拥挤
- C) `el-table` expand 行展开 → 交互稍复杂，与现有页面风格不一致
